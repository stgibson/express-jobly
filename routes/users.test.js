...
describe("GET /users/:username", function () {
  ...
  test("works for getting own info", async function () {
    const resp = await request(app)
        .get(`/users/u2`)
        .set("authorization", `Bearer ${u2Token}`);
    expect(resp.body).toEqual({
...
describe("PATCH /users/:username", () => {
  ...
  test("works for non-admins updating self", async function () {
    const resp = await request(app)
        .patch(`/users/u2`)
        .send({
          firstName: "New"
  ...
  test("unauth for non-admins updating other user", async function () {
    const resp = await request(app)
        .patch(`/users/u1`)
        .send({
          firstName: "New"
...
describe("DELETE /users/:username", function () {
  ...
  test("works for non-admins deleting self", async function () {
    const resp = await request(app)
        .delete(`/users/u2`)
        .set("authorization", `Bearer ${u2Token}`);
    expect(resp.body).toEqual({ deleted: "u2" });
  ...
  test("unauth for non-admins deleting other user", async function () {
    const resp = await request(app)
        .delete(`/users/u1`)
        .set("authorization", `Bearer ${u2Token}`);
    expect(resp.statusCode).toEqual(401);
